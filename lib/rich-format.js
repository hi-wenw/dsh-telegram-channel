...
function normalizeTelegramNativeMarkdown(markdown) {
    const lines = markdown.replace(/\r\n/g, '\n').split('\n');
    let fence;
    let displayMath = false;
    const hasClosingDisplayMathDelimiter = (source, index) => {
        for (let i = index + 1; i < source.length; i++) {
            if (source[i].trim() === '$$')
                return true;
            if (/^ {0,3}(`{3,}|~{3,})/.test(source[i]))
                return false;
        }
        return false;
    };
...
function findTelegramNativeMarkdownSplitIndex(text, hardLimit = RICH_MESSAGE_MAX_CHARS) {
...
export function splitRichMarkdown(markdown) {
    const normalizedMarkdown = normalizeTelegramNativeMarkdown(markdown);
    if (normalizedMarkdown.length <= RICH_MESSAGE_MAX_CHARS
    ...
    for (const rawBlock of splitTelegramNativeMarkdownBlocks(normalizedMarkdown)) {
        for (const block of splitTelegramNativeMarkdownCountedBlocks(rawBlock)) {
            ...
            if (block.length <= RICH_MESSAGE_MAX_CHARS
